...
void EditControl::SetSel(int start, int end)
{
...
bool EditControl::GetEditBinaryNum(u32 &dest)
{
    ...
    {
        int headerLength = 0;
        if (editText.length() > 2 && editText[0] == '0' && (editText[1] == 'b' || editText[1] == 'B'))
            headerLength = 2;
        ...
        int length = editText.length();
        for ( int i=length-1; i>=headerLength; i-- )
        {
            if ( editText[i] == '1' )
...
bool EditControl::GetEditNum(numType &dest)
{
    ...
    if (GetWinText(text) && text.length() > 0)
    {
        ...
        errno = 0;
        char* endPtr = nullptr;
        long long temp = std::strtoll(text.c_str(), &endPtr, base);
        if ( temp != 0 )
        {
            dest = (numType)temp;
            return true;
